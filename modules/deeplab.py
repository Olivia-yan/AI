...
class DeeplabV3(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat([m(x) for m in self.map_convs], dim=1)#out [6,1024,32,32],m为Conv2d(2048, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        #张量m(x)按照行并排起来
        ...
        r = torch.rand(1, out.shape[1], 1, 1, dtype=torch.float32)#创建了一个张量 r，其形状是 (1, out.shape[1], 1, 1)
        if self.training == False:
            r[:,:,:,:] = 1.0

        #这段代码创建了两个与张量 r 形状相同的新张量 weight_out_branch 和 weight_out_new_branch。这些张量的值是通过根据 r 的值分别设置的，具体逻辑如下：
        #对于 weight_out_branch 和 weight_out_new_branch，如果 r 中的值小于 0.33，相应位置的值设置为 2 和 0；如果值在 [0.33, 0.66) 范围内，相应位置的值设置为 0 和 2；如果值大于等于 0.66，相应位置的值设置为 1。
