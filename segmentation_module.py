...
class IncrementalSegmentationModule(nn.Module):
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        #x [6, 3, 512, 512]
        #self.body   5个mod ，每个mod中是ResidualBlock。ResNet(
        #     (mod1): Sequential(
        #     (conv1): Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        # (bn1): InPlaceABNSync(64, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        ...
        # (bn1): InPlaceABNSync(64, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # (extra1): LeakyReLU(negative_slope=0.01)
        # (conv2): Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        # (bn2): InPlaceABNSync(64, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # (dropout): Dropout(p=0.5, inplace=False)
        # (extra2): LeakyReLU(negative_slope=0.01)
        # (conv3): Conv2d(64, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        # (bn3): InPlaceABNSync(256, eps=1e-05, momentum=0.1, affine=True, activation=identity)
        # (conv2_new): Conv2d(64, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        # (bn2_new): InPlaceABNSync(64, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # (dropout_new): Dropout(p=0.5, inplace=False)
        # )
        # (proj_conv): Conv2d(64, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        # (proj_bn): InPlaceABNSync(256, eps=1e-05, momentum=0.1, affine=True, activation=identity)
        # )
        ...
        # (dropout_new): Dropout(p=0.5, inplace=False)
        # )
        # )block3和block2一样；mod3比mod2多一个block4（和block2一样）；mod4 比mod2多很多block（和block2一样），一直到block23；mod5和mod2一样

        x_b, attentions, branch1_x, branch2_x = self.body(x)#x_b ([6, 2048, 32, 32])   attentions, branch1_x, branch2_x都是list
        ...
        # )
        # (map_bn): InPlaceABNSync(1024, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # (map_convs_new): ModuleList(
        #     (0): Conv2d(2048, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        # (1): Conv2d(2048, 256, kernel_size=(3, 3), stride=(1, 1), padding=(6, 6), dilation=(6, 6), bias=False)
        # (2): Conv2d(2048, 256, kernel_size=(3, 3), stride=(1, 1), padding=(12, 12), dilation=(12, 12), bias=False)
        # (3): Conv2d(2048, 256, kernel_size=(3, 3), stride=(1, 1), padding=(18, 18), dilation=(18, 18), bias=False)
        # )
        # (map_bn_new): InPlaceABNSync(1024, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # (global_pooling_conv): Conv2d(2048, 256, kernel_size=(1, 1), stride=(1, 1), bias=False)
        # (global_pooling_bn): InPlaceABNSync(256, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        ...
        # (red_bn): InPlaceABNSync(256, eps=1e-05, momentum=0.1, affine=True, activation=leaky_relu[1.0])
        # )

        # print('head',x_b.shape)

        x_pl = self.head(x_b) #([6, 256, 32, 32])

        # ccnet = CrissCrossAttention(256,x_pl.device)
        # x_pl = ccnet(x_pl)
